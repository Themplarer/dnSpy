...
namespace dnSpy.Hex.Files
{
    sealed class HexBufferFileServiceImpl : HexBufferFileService
    {
        ...
        public HexBufferFileServiceImpl(HexBuffer buffer, Lazy<StructureProviderFactory, VSUTIL.IOrderable>[] structureProviderFactories,
            Lazy<BufferFileHeadersProviderFactory>[] bufferFileHeadersProviderFactories)
        {
            ...
            this.bufferFileHeadersProviderFactories =
                bufferFileHeadersProviderFactories ?? throw new ArgumentNullException(nameof(bufferFileHeadersProviderFactories));
        ...
        public override HexBufferFile[] CreateFiles(BufferFileOptions[] options)
        {
            ...
            for (int i = 0; i < newFiles.Length; i++)
            {
                ...
                newFiles[i] = new HexBufferFileImpl(null, structureProviderFactories, bufferFileHeadersProviderFactories, buffer, opts.Span, opts.Name,
                    opts.Filename, opts.Tags);
            }
