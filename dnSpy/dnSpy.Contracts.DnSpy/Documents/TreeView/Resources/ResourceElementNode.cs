...
public abstract class ResourceElementNode : DocumentTreeNodeData, IResourceNode
{
    ...
    /// </summary>
    public ResourceElement ResourceElement { get; private set; }
    ...
    /// <inheritdoc/>
    public sealed override NodePathName NodePathName => new(Guid, NameUtilities.CleanName(ResourceElement.Name));
    ...
    protected ResourceElementNode(ITreeNodeGroup treeNodeGroup, ResourceElement resourceElement, IDocumentTreeNodeDataContext context)
        : base(context)
    {
    ...
    protected virtual string ValueString
    {
        get
        {
            switch (ResourceElement.ResourceData.Code)
            {
                ...
                default:
                    if (ResourceElement.ResourceData is BinaryResourceData binData)
    ...
    internal static IEnumerable<ResourceData> GetSerializedData(ResourceElement resourceElement)
    {
        ...
        yield return new ResourceData(resourceElement.Name, _ => outStream);
    }
    ...
    public virtual string? CheckCanUpdateData(ResourceElement newResElem) =>
        ResourceElement.ResourceData.Code.FixUserType() != newResElem.ResourceData.Code.FixUserType()
            ? dnSpy_Contracts_DnSpy_Resources.ResourceTypeCantBeChanged
            : string.Empty;
