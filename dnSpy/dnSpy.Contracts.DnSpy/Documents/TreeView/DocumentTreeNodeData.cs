...
public abstract class DocumentTreeNodeData : TreeNodeData
{
    ...
    private bool _refilter;
    private int _filterVersion;
    private List<object>? _dataList;

    protected DocumentTreeNodeData(IDocumentTreeNodeDataContext context) =>
    ...
    public sealed override object? Text
    {
        get
        {
            if (_cachedText?.Target is { } cached)
                return cached;
            ...
            try
            {
                ...
                var classifierContext = new TreeViewNodeClassifierContext(writer.Text, Context.DocumentTreeView.TreeView, this, isToolTip: false,
                    colorize: Context.SyntaxHighlight, colors: writer.Colors);
    ...
    protected bool GetShowToken(DocumentNodeWriteOptions options) =>
        ((options & DocumentNodeWriteOptions.ToolTip) != 0 || (options & DocumentNodeWriteOptions.Title) == 0) && Context.ShowToken;
    ...
    public sealed override object? ToolTip
    {
        get
        {
            ...
            var classifierContext = new TreeViewNodeClassifierContext(writer.Text, Context.DocumentTreeView.TreeView, this, isToolTip: true,
                colorize: Context.SyntaxHighlight, colors: writer.Colors);
    ...
    /// <param name="options">Options</param>
    /// <returns></returns>
    public string ToString(DocumentNodeWriteOptions options) => ToString(Context.Decompiler, options);
    ...
    /// </summary>
    public sealed override void OnRefreshUI() => _cachedText = null;
    ...
    static void Filter(DocumentTreeNodeData? node)
    {
        ...
        switch (res)
        {
            ...
            case FilterType.Visible:
                ...
                if (node is { _refilter: true, TreeNode.Children.Count: > 0 })
    ...
    public sealed override void OnChildrenChanged(TreeNodeData[] added, TreeNodeData[] removed)
    {
        ...
        if (TreeNode is { IsVisible: true, Children.Count: 0 })
            Filter(this);
    }
    ...
    {
        if (_refilter && TreeNode.Children.Count > 0 && TreeNode.IsVisible)
    ...
    public sealed override bool CanDrag(TreeNodeData[] nodes) =>
        Context.CanDragAndDrop &&
        nodes.Length != 0 &&
        nodes.All(a => a is DocumentTreeNodeData data && data.TreeNode.Parent == Context.DocumentTreeView.TreeView.Root);
    ...
    protected void WriteScope(ITextColorWriter output, IScope? scope)
    {
        switch (scope)
        {
            case AssemblyRef asmRef:
                output.Write(asmRef);
                return;
            case ModuleRef modRef:
                output.WriteModule(modRef.Name);
                return;
            case ModuleDef modDef:
    ...
    protected void WriteMemberRef(ITextColorWriter output, IDecompiler decompiler, IMemberRef member)
    {
        ...
        if (member.DeclaringType is { } declType)
        {
            output.WriteLine();
    ...
    {
        private static readonly TextClassifierTextColorWriter Writer = new();
...
public static class DocumentTreeNodeDataExtensionMethods
{
    ...
    {
        var root = self?.TreeNode.TreeView.Root;
    ...
    /// <returns></returns>
    public static ModuleDef? GetModule(this TreeNodeData? self) => self.GetDocumentNode()?.Document.ModuleDef;
    ...
    /// <param name="self">This</param>
    /// <returns></returns>
    public static ModuleDef? GetParentModule(this TreeNodeData? self) => self?.TreeNode.Parent?.Data.GetDocumentNode()?.Document.ModuleDef;
}