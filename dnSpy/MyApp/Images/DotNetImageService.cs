...
namespace MyApp.Images;

internal sealed class DotNetImageService : IDotNetImageService
{
    private static readonly UTF8String StringMicrosoftVisualBasicCompilerServices = new("Microsoft.VisualBasic.CompilerServices");
    private static readonly UTF8String StringStandardModuleAttribute = new("StandardModuleAttribute");
    ...
    public ImageReference GetImageReference(EventDef @event) => ConvertEventToImageReference(@event.AddMethod ?? @event.RemoveMethod);

    public ImageReference GetImageReference(PropertyDef property) => ConvertPropertyToImageReference(property.GetMethod ?? property.SetMethod);
    ...
    private static bool IsModule(TypeDef type) =>
        ...
        type.IsSealed &&
        type.IsDefined(StringMicrosoftVisualBasicCompilerServices, StringStandardModuleAttribute);
    ...
    private static bool IsException(TypeDef type)
    {
        var td = type;
        ...
        for (var i = 0; i < 1000 && td is not null; i++)
        {
            if (IsSystemException(td.BaseType))
                return true;

            td = td.BaseType?.ScopeType.ResolveTypeDef();
        }

        return false;
    }

    private static bool IsSystemException(ITypeDefOrRef? type) =>
        type is not null &&
        type.DeclaringType is null &&
    ...
    private static bool IsDecimalConstant(FieldDef field) =>
        IsSystemDecimal(field.FieldType) &&
        field.CustomAttributes.IsDefined("System.Runtime.CompilerServices.DecimalConstantAttribute");

    private static bool IsSystemDecimal(TypeSig ts) =>
        ts is not null && ts.DefinitionAssembly.IsCorLib() && ts.FullName == "System.Decimal";

    private static ImageReference GetImageReference(Characteristics ch)
    {
        var isExe = (ch & Characteristics.Dll) == 0;
        return isExe ? DsImages.AssemblyExe : DsImages.Assembly;
    }

    private static ImageReference ConvertEventToImageReference(MethodDef? method) =>
        method?.Access switch
        {
            MethodAttributes.Private => DsImages.EventPrivate,
            MethodAttributes.Family => DsImages.EventProtected,
            MethodAttributes.Assembly or MethodAttributes.FamANDAssem => DsImages.EventInternal,
            MethodAttributes.CompilerControlled => DsImages.EventSealed,
            MethodAttributes.FamORAssem => DsImages.EventShortcut,
            _ => DsImages.EventPublic
        };

    private static ImageReference ConvertPropertyToImageReference(MethodDef? method) =>
        method?.Access switch
        {
            MethodAttributes.Private => DsImages.PropertyPrivate,
            MethodAttributes.Family => DsImages.PropertyProtected,
            MethodAttributes.Assembly or MethodAttributes.FamANDAssem => DsImages.PropertyInternal,
            MethodAttributes.CompilerControlled => DsImages.PropertySealed,
            MethodAttributes.FamORAssem => DsImages.PropertyShortcut,
            _ => DsImages.Property
        };
}