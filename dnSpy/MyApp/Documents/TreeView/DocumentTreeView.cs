...
internal sealed class DocumentTreeView : IDocumentTreeView, ITreeViewListener
{
    private readonly DocumentTreeNodeDataContext _context;
    private readonly AssemblyExplorerMostRecentlyUsedList? _mruList;
    private readonly Lazy<IDsDocumentNodeProvider, IDsDocumentNodeProviderMetadata>[] _dsDocumentNodeProvider;
    private readonly Lazy<IDocumentTreeNodeDataFinder, IDocumentTreeNodeDataFinderMetadata>[] _nodeFinders;
    private readonly DocumentTreeNodeGroups _documentTreeNodeGroups;
    private readonly IDecompilerService _decompilerService;
    private readonly IDocumentTreeViewSettings _documentTreeViewSettings;
    private readonly Dispatcher _dispatcher;
    private readonly List<Action> _actionsToCall = new();
    private bool _disableSelectionChanged;

    // ReSharper disable once UnusedMember.Local
    public DocumentTreeView(ITreeViewService treeViewService, IDecompilerService decompilerService, IDsDocumentService documentService,
        ...
        ITreeViewNodeTextElementProvider treeViewNodeTextElementProvider, AssemblyExplorerMostRecentlyUsedList? mruList)
    {
        ...
        _documentTreeViewSettings = documentTreeViewSettings;
        _mruList = mruList;

        _context = new DocumentTreeNodeDataContext(this, resourceNodeFactory, filter ?? FilterNothingDocumentTreeNodeFilter.Instance,
            treeViewNodeTextElementProvider)
        {
        ...
        _documentTreeNodeGroups = new DocumentTreeNodeGroups();
        _dsDocumentNodeProvider = dsDocumentNodeProvider.OrderBy(a => a.Metadata.Order).ToArray();
    ...
    public IDocumentTreeNodeGroups DocumentTreeNodeGroups => _documentTreeNodeGroups;
    ...
    private void CallCollectionChanged(NotifyDocumentTreeViewCollectionChangedEventArgs eventArgs) =>
    ...
    private void TreeView_SelectionChanged(object? sender, TreeViewSelectionChangedEventArgs e)
    {
        if (_disableSelectionChanged)
            return;
    ...
    {
        DocumentService.CollectionChanged -= DocumentService_CollectionChanged;
        _decompilerService.DecompilerChanged -= DecompilerService_DecompilerChanged;
        _documentTreeViewSettings.PropertyChanged -= DocumentTreeViewSettings_PropertyChanged;
    ...
    private void RefilterNodes()
    {
        _context.FilterVersion++;
    ...
    private void InitializeDocumentTreeNodeGroups()
    {
        var orders = new[]
        {
            ...
            _documentTreeViewSettings.MemberKind2,
            _documentTreeViewSettings.MemberKind3,
            _documentTreeViewSettings.MemberKind4,
        };
        _documentTreeNodeGroups.SetMemberOrder(orders);
    }
    ...
    private void CallActions()
    {
        ...
        lock (_actionsToCall)
        {
            list = new List<Action>(_actionsToCall);
    ...
    private void DocumentTreeViewSettings_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        switch (e.PropertyName)
        {
            case nameof(documentTreeViewSettings.SyntaxHighlight):
                _context.SyntaxHighlight = documentTreeViewSettings.SyntaxHighlight;
                RefreshNodes();
                break;

            case nameof(documentTreeViewSettings.ShowAssemblyVersion):
                _context.ShowAssemblyVersion = documentTreeViewSettings.ShowAssemblyVersion;
                RefreshNodes();
                NotifyNodesTextRefreshed();
                break;

            case nameof(documentTreeViewSettings.ShowAssemblyPublicKeyToken):
                _context.ShowAssemblyPublicKeyToken = documentTreeViewSettings.ShowAssemblyPublicKeyToken;
                RefreshNodes();
                NotifyNodesTextRefreshed();
                break;

            case nameof(documentTreeViewSettings.ShowToken):
                _context.ShowToken = documentTreeViewSettings.ShowToken;
                RefreshNodes();
                NotifyNodesTextRefreshed();
                break;

            case nameof(documentTreeViewSettings.SingleClickExpandsTreeViewChildren):
                _context.SingleClickExpandsChildren = documentTreeViewSettings.SingleClickExpandsTreeViewChildren;
                break;

            case nameof(documentTreeViewSettings.DeserializeResources):
                _context.DeserializeResources = documentTreeViewSettings.DeserializeResources;
                break;
        }
    }
    ...
    private void NotifyNodesTextRefreshed() => NodesTextChanged?.Invoke(this, EventArgs.Empty);

    private void DecompilerService_DecompilerChanged(object? sender, EventArgs e) => UpdateDecompiler(((IDecompilerService)sender!).Decompiler);

    private void UpdateDecompiler(IDecompiler newDecompiler)
    {
        _context.Decompiler = newDecompiler;
        RefreshNodes();
        RefilterNodes();
        NotifyNodesTextRefreshed();
    }
    ...
    private void DisableMemoryMappedIo(DsDocumentNode[] nodes)
    {
    ...
    void ITreeViewListener.OnEvent(ITreeView treeView, TreeViewListenerEventArgs e)
    {
        ...
        {
            Debug2.Assert(_context is not null);
    ...
    public DocumentTreeNodeData? FindNode(object? @ref)
    {
        ...
        // foreach (var finder in _nodeFinders)
        // {
    ...
    private DsDocumentNode? Find(IEnumerable<DsDocumentNode> nodes, IDsDocument document)
    {
    ...
    private TypeNode? FindNode(NamespaceNode? nsNode, TypeDef? type)
    {
    ...
    private TypeNode? FindNode(TypeNode? typeNode, TypeDef? type)
    {
    ...
    private IEnumerable<DsDocumentNode> GetAllCreatedDsDocumentNodes(IEnumerable<TreeNodeData> nodes)
    {
        foreach (var n in nodes)
        {
    ...
    private void OnDropNodes(int index, int[] nodeIndexes)
    {
        if (!_context.CanDragAndDrop)
            return;
    ...
    private void OnDropFiles(int index, string[] filenames)
    {
        Debug2.Assert(_mruList is not null);
        if (_mruList is null)
            return;
        if (!_context.CanDragAndDrop)
            return;
        ...
        filenames = filenames.Where(a => File.Exists(a) && !toDoc.ContainsKey(a)).Distinct(StringComparer.OrdinalIgnoreCase)
            .OrderBy(a => Path.GetFileNameWithoutExtension(a), StringComparer.CurrentCultureIgnoreCase).ToArray();
        ...
        for (int i = 0, j = 0; i < filenames.Length; i++)
        {
            ...
            {
                switch (_documentTreeViewSettings.FilterDraggedItems)
                {
    ...
    private DsDocumentNode[]? GetNewSortedNodes()
    {
    ...
    public void SortTopNodes()
    {
        ...
        var old = _disableSelectionChanged;
        ...
        {
            _disableSelectionChanged = old;
        }
    }

    private static bool Equals(IList<DsDocumentNode> a, IList<DsDocumentNode> b)
    {
        if (a.Count != b.Count)
            return false;
    ...
    private sealed class AddDocumentInfo
    {
    ...
    private sealed class DsDocumentNodeComparer : IComparer<(DsDocumentNode Node, int Order)>
    {
        public static readonly DsDocumentNodeComparer Instance = new();

        public int Compare((DsDocumentNode Node, int Order) x, (DsDocumentNode Node, int Order) y)
        {
            if (x.Equals(y))
                return 0;

            if (GetIsAutoLoadedOrder(x.Node).CompareTo(GetIsAutoLoadedOrder(y.Node)) is var nodesCompared && nodesCompared != 0)
                return nodesCompared;

            if (StringComparer.InvariantCultureIgnoreCase.Compare(x.ToString(), y.ToString()) is var stringsCompared && stringsCompared != 0)
                return stringsCompared;

            return x.Order.CompareTo(y.Order);
        }

        private static int GetIsAutoLoadedOrder(DsDocumentNode node) => node.Document.IsAutoLoaded ? 1 : 0;
    }
    ...
    private sealed class GuidObjectsProvider : IGuidObjectsProvider
    {
        ...
        public GuidObjectsProvider(ITreeView treeView) => this._treeView = treeView;

        public IEnumerable<GuidObject> GetGuidObjects(GuidObjectsProviderArgs args)
        {
            yield return new GuidObject(MenuConstants.GUIDOBJ_TREEVIEW_NODES_ARRAY_GUID, _treeView.TopLevelSelection);
        }
    }
}