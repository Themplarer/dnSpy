...
sealed class TypeReferencesFolderNodeImpl : TypeReferencesFolderNode
{
    ...
    public TypeReferencesFolderNodeImpl(ITreeNodeGroup treeNodeGroup, ModuleDocumentNode moduleNode, IDocumentTreeNodeDataContext context)
        : base(context)
    {
    ...
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TypeReferencesFolder"); // todo
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        if (moduleNode.Document.ModuleDef is ModuleDefMD module)
        {
            ...
            foreach (var kv in typeDict)
            {
                ...
                yield return new TypeReferenceNodeImpl(
                    Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.TypeReferenceTreeNodeGroupTypeReferences), type,
                    typeInfo, Context);
            }
        }
    }
...
sealed class TypeSpecsFolderNodeImpl : TypeSpecsFolderNode
{
    ...
    public TypeSpecsFolderNodeImpl(ITreeNodeGroup treeNodeGroup, Dictionary<ITypeDefOrRef, TypeRefInfo> typeDict, IDocumentTreeNodeDataContext context)
        : base(context)
    {
        TreeNodeGroup = treeNodeGroup;
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        Debug2.Assert(typeDict is not null);
        ...
        foreach (var kv in typeDict)
        {
            ...
            yield return new TypeReferenceNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.TypeSpecTreeNodeGroupTypeSpecsFolder), type,
                typeInfo, Context);
        }
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TreeViewTypesFolder"); // todo
...
sealed class MethodReferencesFolderNodeImpl : MethodReferencesFolderNode
{
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        Debug2.Assert(methodDict is not null);

        foreach (var kv in methodDict)
        {
            var method = kv.Key;
            yield return new MethodReferenceNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.MethodReferenceTreeNodeGroupMethodReferencesFolder),
                method, Context);
        }
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TreeViewMethodsFolder"); // todo
...
sealed class PropertyReferencesFolderNodeImpl : PropertyReferencesFolderNode
{
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        Debug2.Assert(propertyDict is not null);

        foreach (var kv in propertyDict)
        {
            var property = kv.Key;
            yield return new PropertyReferenceNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType
                    .PropertyReferenceTreeNodeGroupPropertyReferencesFolder), property, Context);
        }
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TreeViewPropertiesFolder"); // todo
...
sealed class EventReferencesFolderNodeImpl : EventReferencesFolderNode
{
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        Debug2.Assert(eventDict is not null);

        foreach (var kv in eventDict)
        {
            var @event = kv.Key;
            yield return new EventReferenceNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.EventReferenceTreeNodeGroupEventReferencesFolder),
                @event, Context);
        }
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TreeViewEventsFolder"); // todo
...
sealed class FieldReferencesFolderNodeImpl : FieldReferencesFolderNode
{
    ...
    public FieldReferencesFolderNodeImpl(ITreeNodeGroup treeNodeGroup, Dictionary<MemberRef, MemberRef> fieldDict, IDocumentTreeNodeDataContext context)
        : base(context)
    {
        TreeNodeGroup = treeNodeGroup;
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        Debug2.Assert(fieldDict is not null);

        foreach (var kv in fieldDict)
        {
            var field = kv.Key;
            yield return new FieldReferenceNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.FieldReferenceTreeNodeGroupFieldReferencesFolder),
                field, Context);
        }
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        output.Write(BoxedTextColor.Text, "dnSpy_Resources.TreeViewFieldsFolder"); // todo
...
sealed class TypeReferenceNodeImpl : TypeReferenceNode
{
    ...
    public TypeReferenceNodeImpl(ITreeNodeGroup treeNodeGroup, ITypeDefOrRef type, TypeRefInfo typeInfo, IDocumentTreeNodeDataContext context)
        : base(type, context)
    {
        TreeNodeGroup = treeNodeGroup;
    ...
    {
        Debug2.Assert(typeInfo is not null);
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        ...
        if (typeInfo.TypeDict.Count != 0)
            yield return new TypeSpecsFolderNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.TypeSpecsFolderTreeNodeGroupTypeReference),
                typeInfo.TypeDict, Context);
        if (typeInfo.MethodDict.Count != 0)
            yield return new MethodReferencesFolderNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.MethodReferencesFolderTreeNodeGroupTypeReference),
                typeInfo.MethodDict, Context);
        if (typeInfo.FieldDict.Count != 0)
            yield return new FieldReferencesFolderNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.FieldReferencesFolderTreeNodeGroupTypeReference),
                typeInfo.FieldDict, Context);
        if (typeInfo.PropertyDict.Count != 0)
            yield return new PropertyReferencesFolderNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.PropertyReferencesFolderTreeNodeGroupTypeReference),
                typeInfo.PropertyDict, Context);
        if (typeInfo.EventDict.Count != 0)
            yield return new EventReferencesFolderNodeImpl(
                Context.DocumentTreeView.DocumentTreeNodeGroups.GetGroup(DocumentTreeNodeGroupType.EventReferencesFolderTreeNodeGroupTypeReference),
                typeInfo.EventDict, Context);
...
sealed class MethodReferenceNodeImpl : MethodReferenceNode
{
    ...
    public MethodReferenceNodeImpl(ITreeNodeGroup treeNodeGroup, IMethod method, IDocumentTreeNodeDataContext context)
        : base(method, context) => TreeNodeGroup = treeNodeGroup;
...
sealed class PropertyReferenceNodeImpl : PropertyReferenceNode
{
    ...
    public PropertyReferenceNodeImpl(ITreeNodeGroup treeNodeGroup, IMethod method, IDocumentTreeNodeDataContext context)
        : base(method, context) => TreeNodeGroup = treeNodeGroup;
    ...
    protected override void WriteCore(ITextColorWriter output, IDecompiler decompiler, DocumentNodeWriteOptions options)
    {
        if ((options & DocumentNodeWriteOptions.ToolTip) != 0)
        {
            WriteMemberRef(output, decompiler, TryResolveDef(out var property, out _) ? property : PropertyRef);
            output.WriteLine();
...
sealed class EventReferenceNodeImpl : EventReferenceNode
{
    ...
    public EventReferenceNodeImpl(ITreeNodeGroup treeNodeGroup, IMethod method, IDocumentTreeNodeDataContext context)
        : base(method, context) => TreeNodeGroup = treeNodeGroup;
...
sealed class FieldReferenceNodeImpl : FieldReferenceNode
{
    ...
    protected override ImageReference GetIcon(IDotNetImageService dnImgMgr)
    {
        if (imageReference is not null) return imageReference.Value;

        if (FieldRef.ResolveFieldDef() is { } fd)
    ...
    public FieldReferenceNodeImpl(ITreeNodeGroup treeNodeGroup, MemberRef field, IDocumentTreeNodeDataContext context)
        : base(field, context) => TreeNodeGroup = treeNodeGroup;
