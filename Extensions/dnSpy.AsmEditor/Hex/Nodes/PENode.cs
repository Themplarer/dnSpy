...
sealed class PENode : DocumentTreeNodeData, IDecompileSelf
{
    ...
    public PENode(Func<HexBufferFile> createBufferFile, PEStructureProviderFactory peStructureProviderFactory, IDocumentTreeNodeDataContext context)
        : base(context)
    {
    ...
    public override IEnumerable<TreeNodeData> CreateChildren()
    {
        ...
        yield return new ImageDosHeaderNode(peStructureProvider.ImageDosHeader, Context);
        yield return new ImageFileHeaderNode(peStructureProvider.ImageFileHeader, Context);

        if (peStructureProvider.ImageOptionalHeader is ImageOptionalHeader32VM vm)
            yield return new ImageOptionalHeader32Node(vm, Context);
        else
            yield return new ImageOptionalHeader64Node((ImageOptionalHeader64VM)peStructureProvider.ImageOptionalHeader, Context);

        for (var i = 0; i < peStructureProvider.Sections.Length; i++)
            yield return new ImageSectionHeaderNode(peStructureProvider.Sections[i], i, Context);

        var cor20Hdr = ImageCor20HeaderNode.Create(peStructureProvider.ImageCor20Header, Context);

        if (cor20Hdr is null) yield break;

        yield return cor20Hdr;

        if (peStructureProvider.StorageSignature is null) yield break;

        yield return new StorageSignatureNode(peStructureProvider.StorageSignature, Context);
        yield return new StorageHeaderNode(peStructureProvider.StorageHeader!, Context);

        foreach (var storageStream in peStructureProvider.StorageStreams)
            if (storageStream.HeapKind == DotNetHeapKind.Tables)
                yield return new TablesStorageStreamNode(storageStream, peStructureProvider.TablesStream!, Context);
            else
                yield return new StorageStreamNode(storageStream, Context);
    }
    ...
    public MetadataTableRecordNode? FindTokenNode(uint token)
    {
        ...
        TreeNode.EnsureChildrenLoaded();
        var stgStreamNode =
            (StorageStreamNode?)TreeNode.DataChildren.FirstOrDefault(a =>
                a is StorageStreamNode && ((StorageStreamNode)a).HeapKind == DotNetHeapKind.Tables);
