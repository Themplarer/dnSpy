...
sealed class MetadataTableRecordNode : HexNode
{
    ...
    public MetadataTableRecordNode(TableInfo tableInfo, int index, HexPosition startOffset, HexPosition endOffset,
        ...
        : base(HexSpan.FromBounds(startOffset, endOffset), context)
    {
        this.index = index;
    ...
    {
        if (infoTuple is null) return;

        var tableInfo = ((MetadataTableNode)TreeNode.Parent!.Data).TableInfo;

        foreach (var i in infoTuple.Value.Item1)
        {
    ...
    (int[], Action<ITextColorWriter>)? GetInfoTuple(TableInfo tableInfo)
    {
        ...
        {
            case Table.Module: return (new[] { 1 }, WriteModuleInfo);
            case Table.TypeRef: return (new[] { 1, 2 }, WriteTypeRefInfo);
            case Table.TypeDef: return (new[] { 1, 2 }, WriteTypeDefInfo);
            case Table.FieldPtr: return null;
            case Table.Field: return (new[] { 1 }, WriteFieldInfo);
            case Table.MethodPtr: return null;
            case Table.Method: return (new[] { 3 }, WriteMethodInfo);
            case Table.ParamPtr: return null;
            case Table.Param: return (new[] { 2 }, WriteParamInfo);
            case Table.InterfaceImpl: return null;
            case Table.MemberRef: return (new[] { 1 }, WriteMemberRefInfo);
            ...
            case Table.Event: return (new[] { 1 }, WriteEventInfo);
            case Table.PropertyMap: return null;
            case Table.PropertyPtr: return null;
            case Table.Property: return (new[] { 1 }, WritePropertyInfo);
            case Table.MethodSemantics: return null;
            case Table.MethodImpl: return null;
            case Table.ModuleRef: return (new[] { 0 }, WriteModuleRefInfo);
            case Table.TypeSpec: return null;
            case Table.ImplMap: return (new[] { 2 }, WriteImplMapInfo);
            case Table.FieldRVA: return null;
            case Table.ENCLog: return null;
            case Table.ENCMap: return null;
            case Table.Assembly: return (new[] { 7 }, WriteAssemblyInfo);
            case Table.AssemblyProcessor: return null;
            case Table.AssemblyOS: return null;
            case Table.AssemblyRef: return (new[] { 6 }, WriteAssemblyRefInfo);
            case Table.AssemblyRefProcessor: return null;
            case Table.AssemblyRefOS: return null;
            case Table.File: return (new[] { 1 }, WriteFileInfo);
            case Table.ExportedType: return (new[] { 2, 3 }, WriteExportedTypeInfo);
            case Table.ManifestResource: return (new[] { 2 }, WriteManifestResourceInfo);
            case Table.NestedClass: return null;
            case Table.GenericParam: return (new[] { 3 }, WriteGenericParamInfo);
